...
static inline int find_mt_idx(const int * my_mt, int n_my_mt, int mt) {
    for (int j = 0; j < n_my_mt; ++j) {
        if (my_mt[j] == mt) return j;
    }
    return 0;
}

static inline conv_tile_t decode_tile(int t, int M_TILES, int w_tiles,
                                      const int * my_mt, int n_my_mt) {
    conv_tile_t tile;
    tile.mt = t % M_TILES;
    t /= M_TILES;
...
static inline uint64_t filter_slab_addr(uint64_t l2_filter,
                                        int Kw, int K_TILES,
                                        int n_my_mt, int mt_idx,
                                        int kh, int kw, int kt_global) {
    return l2_filter
         + (uint64_t)((((kh * Kw + kw) * n_my_mt + mt_idx) * K_TILES
                     + kt_global)) * SLAB_BYTES;
}

static inline uint64_t pin_tile_addr(uint64_t l2_pad_in, uint64_t pin_copy_bytes,
                                     int ktc, int kw, int Hp, int Wp_a,
                                     int oh, int ow_base, int s1, int kh) {
    const int ir_pad = oh * s1 + kh;
    return l2_pad_in
         + (uint64_t)kw * pin_copy_bytes
         + (((uint64_t)(ktc * TILE) * Hp + ir_pad) * Wp_a + ow_base)
         * sizeof(float);
}
...
static inline char * output_tile_addr(char * out_base,
    ...
    return out_base
         + (size_t)(tile->mt * TILE) * out_chan_stride
         + (size_t)tile->oh          * out_row_stride
...
static inline void compute_tile_chunk(uint64_t l2_filter,
                                      ...
                                      int Kh, int Kw, int K_TILES,
                                      int chunk_KT, int kt_base,
                                      int n_my_mt,
                                      int Hp, int Wp_a, int s1,
    ...
    const uint64_t a_addr0 =
        filter_slab_addr(l2_filter, Kw, K_TILES, n_my_mt,
                         tile->mt_idx, 0, 0, kt_base);
    tensor_load(false, false, A_BUFS[0], 0, 0, a_addr0, 0,
                (uint64_t)(TILE - 1), a_row_stride, 0);
    ...
    for (int iter = 0; iter < n_iters; ++iter) {
        ...
        const uint64_t b_addr =
            pin_tile_addr(l2_pad_in, pin_copy_bytes, ktc, kw,
                          Hp, Wp_a, tile->oh, tile->ow_base, s1, kh);
        tensor_load(false, false, LSCP_B, 0, 0, b_addr, 0,
                    (uint64_t)(TILE - 1), b_row_stride, 1);
        ...
        if (iter + 1 < n_iters) {
            ...
            const uint64_t a_addr_n =
                filter_slab_addr(l2_filter, Kw, K_TILES, n_my_mt,
                                 tile->mt_idx, kh_n, kw_n, kt_base + ktc_n);
            tensor_load(false, false, A_BUFS[(iter + 1) & 1],
                        0, 0, a_addr_n, 0,
                        (uint64_t)(TILE - 1), a_row_stride, 0);
        }

        tensor_fma(false, 3,
                   (uint64_t)(TILE - 1), (uint64_t)(TILE - 1),
                   0, false, false, false, false,
                   LSCP_B, A_BUFS[iter & 1],
                   0, first_fma_clears_tenc && (iter == 0));
...
static void pack_filter_local_mt(const float * flt_base,
                                 int Kh, int Kw, int Cin, int K_TILES,
                                 const int * my_mt, int n_my_mt,
                                 int minion,
    ...
    for (int slab = minion; slab < n_slabs; slab += N_MIN_PER_SHIRE) {
        int t = slab;
        const int kt     = t % K_TILES;  t /= K_TILES;
        const int mt_idx = t % n_my_mt;  t /= n_my_mt;
        ...
        for (int oc_in = 0; oc_in < TILE; ++oc_in) {
            ...
            const float * src = flt_base +
                (((size_t)oc * Cin + (size_t)kt * TILE) * Kh + kh) * Kw + kw;
    ...
    for (int slab = minion; slab < n_slabs; slab += N_MIN_PER_SHIRE) {
        const uint64_t slab_offset = (uint64_t)slab * SLAB_BYTES;
        flush_to_l2((const void *)(l2_filter_base + slab_offset),
                    SLAB_LINES, 64);
    }
    WAIT_CACHEOPS;
}
...
static void pack_pin_chunk(const pin_ctx_t * ctx, int chunk_id, int buf_idx) {
    ...
    for (int p = minion; p < n_pin_planes; p += N_MIN_PER_SHIRE) {
        ...
        int vlo = 0;
        while (vlo < Wp_a && (s0 * vlo + offset) < 0) vlo++;
        int vhi = Wp_a;
        while (vhi > vlo && (s0 * (vhi - 1) + offset) >= W) vhi--;
        const bool aligned = (s0 == 1)
            && ((vlo & 7) == 0)
            && (((vlo + offset) & 7) == 0);
        ...
        for (int r = 0; r < Hp; ++r) {
            ...
            const float * src_row = ctx->in_base +
                ((size_t)ic * H + real_h) * W;

            for (int cc = 0; cc < vlo; ++cc) row[cc] = 0.0f;
            ...
            if (aligned) {
                ...
                const int n = vhi - vlo;
                for (int cc = 0; cc < n; ++cc) row[vlo + cc] = csrc[cc];
            } else {
            ...
            for (int cc = vhi; cc < Wp_a; ++cc) row[cc] = 0.0f;
        }
    }

    /* Flush this buffer's L1D-dirty lines down to L2SCP backing. */
    FENCE;
    flush_range_to_l2((const void *)ctx->l2_pad_in_buf[buf_idx],
                      ctx->pin_chunk_bytes);
    WAIT_CACHEOPS;
}
...
int entry_point(struct ggml_et_binary_params* params, void* env) {
    ...
    if (Cin <= 0 || Cout <= 0)                      return -1;
    if (Cin % TILE != 0 || Cout % TILE != 0)        return -1;
    if (W <= 0 || H <= 0)                           return -1;
    if (s0 <= 0 || s1 <= 0)                         return -1;
    if (in->ne[2] != Cin || in->ne[3] != 1)         return -1;
    if (out->ne[2] != Cout || out->ne[3] != 1)      return -1;
    if (!flt->data || !in->data || !out->data)      return -1;
    ...
    const int n_active_shires = need_stage ? 1
                              : min_i32(total_tiles, N_SHIRES);
    ...
    if (shire >= n_active_shires) return 0;
    ...
    for (int t = shire; t < total_tiles; t += n_active_shires) {
        ...
        bool found = false;
        for (int j = 0; j < n_my_mt; ++j) {
            if (my_mt[j] == mt) { found = true; break; }
        }
        if (!found) {
            if (n_my_mt >= MAX_MY_MT) return -1;
            my_mt[n_my_mt++] = mt;
        }
    }
    if (n_my_mt == 0) return 0;   /* no tiles for this shire */

    const uint64_t filter_local_bytes =
        (uint64_t)Kh * Kw * n_my_mt * K_TILES * SLAB_BYTES;
    if (filter_local_bytes > LOCAL_FILTER_CAP) return -1;
    ...
    const int64_t output_stage_bytes_full = need_stage
        ? (int64_t)Cout * OH * OW_pad * (int64_t)sizeof(float)
        : 0;
    const int64_t budget_for_chunks = (int64_t)LOCAL_BUDGET
                                    - (int64_t)filter_local_bytes
                                    - output_stage_bytes_full;
    if (budget_for_chunks <= 0) return -1;
    const int64_t per_KT_pin_bytes =
        (int64_t)Kw * TILE * Hp * Wp_a * (int64_t)sizeof(float);
    ...
    if ((int64_t)K_TILES * per_KT_pin_bytes <= budget_for_chunks) {
        ...
        chunk_KT = K_TILES;
        while (chunk_KT > 1 &&
               2 * (int64_t)chunk_KT * per_KT_pin_bytes > budget_for_chunks) {
            chunk_KT--;
        }
        while (chunk_KT > 1 && K_TILES % chunk_KT != 0) chunk_KT--;
        n_buffers = (chunk_KT < K_TILES) ? 2 : 1;
        if (chunk_KT < 1) return -1;
    }
    ...
    const int shire_tile_capacity =
        shire + MAX_TILES_PER_HART * n_active_shires * N_MIN_PER_SHIRE;
    if (n_chunks > 1 && shire_tile_capacity < total_tiles) return -1;
    ...
    const uint64_t l2_output_stage = need_stage
        ? l2_pin_base + (uint64_t)n_buffers * pin_chunk_bytes
        : 0;

    const uint64_t scratch_per_hart = (uint64_t)MAX_TILES_PER_HART
                                    * (uint64_t)TILE * TILE * sizeof(float);
    const uint64_t l2_scratch_base  = need_stage
        ? l2_output_stage + (uint64_t)output_stage_bytes_full
        : l2_pin_base + (uint64_t)n_buffers * pin_chunk_bytes;
    ...
    if (hart1) {
        pack_filter_local_mt((const float *)flt->data,
                             Kh, Kw, Cin, K_TILES,
                             my_mt, n_my_mt,
                             minion, l2_filter);
    }
    ...
    if (hart1) {
        const pin_ctx_t ctx = {
            ...
            .Kw = Kw,
            .chunk_KT        = chunk_KT,
            .H = H, .W = W, .Hp = Hp, .Wp_a = Wp_a,
            .pad_h = pad_h, .pad_w = pad_w, .s0 = s0,
            .minion          = minion,
        ...
        if (need_stage) et_barrier(ET_BARRIER_SHIRE);
        return 0;
    }
    ...
    char * const out_base = need_stage
        ? (char *)l2_output_stage
        : (char *)out->data;
    const int compute_OW = need_stage ? OW_pad : OW;
    const uint64_t out_chan_stride =
        (uint64_t)OH * (uint64_t)compute_OW * sizeof(float);
    const uint64_t out_row_stride  =
        (uint64_t)compute_OW * sizeof(float);
    ...
    if (n_chunks == 1) {
        ...
        for (int t = t_start; t < total_tiles; t += t_stride) {
            const conv_tile_t tile =
                decode_tile(t, M_TILES, w_tiles, my_mt, n_my_mt);
            compute_tile_chunk(l2_filter, l2_pad_in, pin_copy_bytes,
                               Kh, Kw, K_TILES, chunk_KT, 0, n_my_mt,
                               Hp, Wp_a, s1, a_row_stride, b_row_stride,
                               &tile, /*first_fma_clears_tenc=*/true);

            char * dst_addr =
                output_tile_addr(out_base, &tile, out_chan_stride, out_row_stride);
            tensor_store(0, 0, 3, (uint64_t)(TILE - 1),
                         (uint64_t)dst_addr, 0,
                         out_chan_stride);
        ...
        for (int slot = 0; slot < MAX_TILES_PER_HART; ++slot) {
            ...
            if (t < total_tiles) my_tiles[n_my_tiles++] = t;
        }

        conv_tile_t tiles[MAX_TILES_PER_HART];
        for (int k = 0; k < n_my_tiles; ++k) {
            tiles[k] = decode_tile(my_tiles[k], M_TILES, w_tiles,
                                   my_mt, n_my_mt);
        }

        const uint64_t my_scratch_base =
            l2_scratch_base + (uint64_t)minion * scratch_per_hart;
        ...
        for (int c = 0; c < n_chunks; ++c) {
            ...
            for (int k = 0; k < n_my_tiles; ++k) {
                ...
                const uint64_t scr =
                    my_scratch_base + (uint64_t)k * (TILE * TILE * sizeof(float));
                ...
                compute_tile_chunk(l2_filter, l2_pad_in, pin_copy_bytes,
                                   Kh, Kw, K_TILES, chunk_KT, kt_base, n_my_mt,
                                   Hp, Wp_a, s1, a_row_stride, b_row_stride,
                                   tile, first_pass_chunk);

                if (c == n_chunks - 1) {
                    char * dst_addr =
                        output_tile_addr(out_base, tile,
                                         out_chan_stride, out_row_stride);
                    tensor_store(0, 0, 3, (uint64_t)(TILE - 1),
                                 (uint64_t)dst_addr, 0,
                                 out_chan_stride);
                } else {
        ...
        if (minion == 0) {
            ...
            for (int oc = 0; oc < Cout; ++oc) {
                for (int oh2 = 0; oh2 < OH; ++oh2) {
                    const float * src =
                        stage + ((size_t)oc * OH + oh2) * OW_pad;
                    float * dst =
                        dram + ((size_t)oc * OH + oh2) * OW;
            ...
            const uint64_t total_bytes =
                (uint64_t)Cout * OH * OW * sizeof(float);
