...
void update_online_softmax_values_internal(const uint32_t dst_index_in0, const uint32_t dst_index_in1, const uint32_t dst_index_out) {
    ...
    const uint32_t in_base_idx = 0 * n_vector_in_tile;
    const uint32_t sum_base_idx = 1 * n_vector_in_tile;
    const uint32_t max_base_idx = 2 * n_vector_in_tile;
...
static void update_online_softmax_values() {
    ...
    MATH(_llk_math_eltwise_binary_sfpu_params_<false>(update_online_softmax_values_internal, 0, 0, 0));
}
...
void MAIN {
    ...
    for(uint32_t y = 0; y < height_tiles; ++y) {
        for(uint32_t x = 0; x < width_tiles; ++x) {
            ...
            copy_tile(cb_sum, 0, 1); // Tile 1 -> Sum
            copy_tile_init(cb_max);
            copy_tile(cb_max, 0, 2); // Tile 2 -> max
        ...
        {
            tile_regs_acquire();
            reduce_init<PoolType::MAX, ReduceDim::REDUCE_COL>(cb_max, cb_const1, cb_tmp);
            reduce_tile<PoolType::MAX, ReduceDim::REDUCE_COL>(cb_max, cb_const1, 0, 0, 0);
            reduce_uninit();
            tile_regs_commit();
            tile_regs_wait();
            pack_tile(0, cb_tmp);
            ...
            unary_bcast_init<BroadcastType::COL>(cb_tmp, cb_tmp);
            unary_bcast<BroadcastType::COL>(cb_tmp, 0, 0);
            sub_binary_tile(1, 0, 2);
            exp_tile(2);
            mul_binary_tile(0, 2, 3);
            reduce_init<PoolType::SUM, ReduceDim::REDUCE_COL>(cb_sum, cb_const1, cb_tmp);
            reduce_tile_math<PoolType::SUM, ReduceDim::REDUCE_COL>(3);
            reduce_uninit();
            tile_regs_commit();
            tile_regs_wait();
            pack_tile(3, cb_tmp);
