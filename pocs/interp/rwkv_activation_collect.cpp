...
llama_interp::task<> capture_token(llama_interp::runtime &          runtime,
    ...
    if (request_captures) {
        for (const std::string & tap : taps) {
            call.capture_f32("^" + tap + "$", captures);
        }
    }
    after = co_await call;
}

llama_interp::task<> prefill_state(llama_interp::runtime &          runtime,
...
void usage(const char * argv0) {
    std::fprintf(stderr,
                 "usage: %s -m MODEL --corpus FILE --output FILE --tap FULL_NAME [--tap FULL_NAME ...] [--max-samples "
                 "N] [--positions-per-line N] [--prompt-stride N] [--context-size N] [--capture-batch N] "
                 "[--commit-rows N] [--seed N] [-ngl N]\n"
                 "\n"
...
int main(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (std::strcmp(argv[i], "--context-size") == 0 && i + 1 < argc) {
            context_size = std::strtoul(argv[++i], nullptr, 10);
        } else if (std::strcmp(argv[i], "--capture-batch") == 0 && i + 1 < argc) {
            capture_batch = std::strtoul(argv[++i], nullptr, 10);
        } else if (std::strcmp(argv[i], "--commit-rows") == 0 && i + 1 < argc) {
            commit_rows = std::strtoull(argv[++i], nullptr, 10);
    ...
    if (model_path.empty() || corpus_path.empty() || output_path.empty() || taps.empty() || duplicate_taps ||
        max_samples == 0 || positions_per_line == 0 || prompt_stride == 0 || context_size < 2 || capture_batch == 0 ||
        commit_rows == 0) {
        usage(argv[0]);
        return 1;
    }
    ...
    context_params.n_batch              = capture_batch;
    context_params.n_ubatch             = capture_batch;
    ...
    const auto flush = [&] {
        ...
        for (size_t i = 0; i < pending.size(); ++i) {
            calls.emplace_back(
                capture_token(runtime, pending[i].state, pending[i].token, taps, i == 0, after[i], captures));
        }
        runtime.run();
        ...
        for (const auto & capture : captures) {
            if (!capture.data_f32.empty() && !by_name.emplace(capture.name, &capture).second) {
        ...
        for (const std::string & tap : taps) {
            ...
            if (found == by_name.end() || found->second->data_f32.size() % pending.size() != 0) {
                throw std::runtime_error("missing or malformed capture: " + tap);
            }
            const size_t tap_dimension = found->second->data_f32.size() / pending.size();
        ...
        if (!writer) {
            writer = std::make_unique<rwkv_activation_store::activation_dataset_writer>(
        ...
        for (size_t sample = 0; sample < pending.size(); ++sample) {
            std::vector<std::span<const float>> activations;
            activations.reserve(taps.size());
            for (const std::string & tap : taps) {
                const auto & values = by_name.at(tap)->data_f32;
        ...
        if (since_commit >= commit_rows || sample_id == max_samples) {
            ...
            std::printf("progress=%llu/%llu %.1f%% corpus_line=%llu rate=%.3f samples/s\n",
                        (unsigned long long) sample_id, (unsigned long long) max_samples,
                        100.0 * sample_id / max_samples, (unsigned long long) corpus_line, rate);
            std::fflush(stdout);
        }
    };
    ...
    while (sample_id + pending.size() < max_samples && std::getline(corpus, prompt)) {
        ++corpus_line;
        if (prompt.empty() || corpus_line % prompt_stride != 1 % prompt_stride) {
            continue;
        }
        ...
        const size_t available_tokens = std::min<size_t>(tokens.size(), context_size);
        if (available_tokens == 0) {
            continue;
        }
        const size_t first_position = std::min<size_t>(8, available_tokens - 1);
        const size_t range          = available_tokens - first_position;
        ...
        for (size_t offset = 0; offset < positions && sample_id + pending.size() < max_samples; ++offset) {
            ...
            if (position > 0) {
                auto prefix = prefill_state(runtime, initial,
                                            std::vector<llama_token>(tokens.begin(), tokens.begin() + position), state);
                runtime.run();
            ...
            pending.push_back({
                std::move(state),
                tokens[position],
                { sample_id + pending.size(), corpus_line, (int32_t) position, tokens[position],
                            (int32_t) tokens.size() }
            });
    ...
    flush();
    if (!writer) {
        throw std::runtime_error("corpus produced no capture samples");
    }
    writer->commit();
    writer.reset();
